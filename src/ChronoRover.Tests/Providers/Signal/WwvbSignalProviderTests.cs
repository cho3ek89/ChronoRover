...
[TestFixture]
public class WwvbSignalProviderTests
{
    [Test]
    ...
    {
        var bits = GetMinuteSignal(date);
        var bitsYear = bits[45..49].Concat(bits[50..54]).ToArray();

        Assert.That(bitsYear, Is.EqualTo(expectedBits));
    }

    [Test]
    [TestCaseSource(nameof(GetLeapYearBitIsCorrectTestData))]
    public void LeapYearBitIsCorrect(DateTime date, bool expectedBit)
    {
        var bits = GetMinuteSignal(date);
    ...
    public void DstBitsAreCorrect(DateTime date, bool[] expectedBits)
    {
        ...
        Assert.That(bits[57..59], Is.EqualTo(expectedBits));
    }
    ...
    public void DateTimeAgnosticBitsAreCorrect(DateTime date)
    {
        ...
        Assert.That(bits[34..37], Has.All.EqualTo(false));
        Assert.That(bits[37], Is.EqualTo(true));
        Assert.That(bits[38..45], Has.All.EqualTo(false));
        Assert.That(bits[49], Is.EqualTo(false));
        Assert.That(bits[54], Is.EqualTo(false));
    ...
    {
        var provider = new WwvbSignalProvider(
    ...
    private static object[] GetLeapYearBitIsCorrectTestData()
    {
        object[] y1 = [GetDate(2023), false]; // not leap year
        object[] y2 = [GetDate(2024), true]; // leap year

        return [y1, y2];

        static DateTime GetDate(int year) => new(year, 1, 1);
    }

    private static object[] GetDstBitsAreCorrectTestData()
    {
        object[] tc1 = [GetDate(3), new[] { false, false }]; // no DST
        object[] tc2 = [GetDate(4), new[] { true, true }]; // is DST
