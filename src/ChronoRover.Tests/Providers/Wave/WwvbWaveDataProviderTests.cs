...
[TestFixture]
public class WwvbWaveDataProviderTests
{
    [Test]
    ...
    public void WaveDataIsCorrectForSecond(int second, bool[] signalBits, string expectedSamplesFileName)
    {
        ...
        var provider = new WwvbWaveDataProvider(
    ...
    {
        var filePath = Path.Combine("TestData", "Wave", "Providers", "WwvbWaveDataProviderTests", fileName);
    ...
    private static object[] GetWaveDataIsCorrectForSecondTestData()
    {
        ...
        {
            tcs[sec * 2] = new object[]
            {
                sec,
                GetSignalBits(),
                IsMarkerSecond(sec) ? "SamplesForMarker.csv" : "SamplesFor0.csv",
            };

            tcs[sec * 2 + 1] = new object[]
            {
                sec,
                GetSignalBits(sec),
                IsMarkerSecond(sec) ? "SamplesForMarker.csv" : "SamplesFor1.csv",
            };
        }

        return tcs;

        static bool IsMarkerSecond(int second) => second is 0 or 9 or 19 or 29 or 39 or 49 or 59;
    }
